...
if admin_mode:
    ...
    match_name = st.sidebar.text_input("輸入對賽場次 (例: 墨西哥 🆚 南非)")
...
with tab1:
    ...
    if not df_rank.empty:
        ...
        if lowest_score < 0: st.error(f"🚨 目前由 **{lowest_player}** 以 {lowest_score} 分領先成為【終極冥燈閪】！")
...
with tab2:
    ...
    if players_list:
        ...
        if not df_matches.empty:
            ...
            st.success("投注已同步。")
    else: st.warning("⚠️ 請先在 Google Sheet 輸入人名。")
