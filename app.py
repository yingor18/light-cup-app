...
try:
    ...
    # 強制轉換成純 Python List，切斷所有同 Pandas Series 嘅關聯
    if not df_players.empty and '人名' in df_players.columns:
        all_players = [str(x).strip() for x in df_players["人名"].dropna().tolist()]
    else:
        all_players = []

    if df_bets.empty:
        st.warning("⚠️ 偵測到 FinalBets 是空的，或者未能成功讀取。")
    ...
    def get_points(row):
        if hasattr(row, 'get'):
            user_choice = ""
            for k in ['選擇', '投注', '盤口', '盤口_x', '盤口_y']:
                if k in row.index and pd.notna(row[k]):
                    user_choice = str(row[k]).strip()
                    break
            
            match_result = ""
            if target_res_col in row.index:
    ...
    if not df_bets.empty and not df_matches.empty and '場次' in df_bets.columns:
        df_bets_clean = df_bets.copy()
        df_bets_clean['人名'] = df_bets_clean['人名'].astype(str).str.strip() if '人名' in df_bets_clean.columns else ""
        ...
        # 安全選擇欄位
        keep_cols = ['乾淨場次', target_res_col]
        for c in ['球隊', '讓球球隊', '盤口', '賽果分數', '時間']:
            if c in df_matches_clean_base.columns:
                keep_cols.append(c)

        # 合併並用新邏輯計分
        merged = df_bets_clean.merge(df_matches_clean_base[keep_cols], on='乾淨場次', how='left')
        merged['得分'] = merged.apply(get_points, axis=1)
        
        if '人名' in merged.columns:
            # 總分統計
    ...
    df_sidebar_unplayed = pd.DataFrame()
    available_matches = []
    
    if not df_matches.empty and target_res_col in df_matches.columns:
        ...
        with st.sidebar.form("bet_form", clear_on_submit=True):
            ...
            if st.form_submit_button("🔥 提交"):
                if type(u) is not str or u.strip() == "":
                    ...
                    if not df_current.empty and '人名' in df_current.columns and '場次' in df_current.columns:
                        if not df_current[(df_current['人名'].astype(str) == u) & (df_current['場次'].astype(str) == m)].empty:
                            st.error("❌ 呢場你投過喇，唔准改！")
                        else:
    ...
    with tab3:
        if not df_bets.empty and '場次' in df_bets.columns:
            st.subheader("📋 按場次查看手足落注")
            all_bet_matches = df_bets['場次'].astype(str).unique().tolist()
            
            default_idx = 0
            if not df_sidebar_unplayed.empty and '場次' in df_sidebar_unplayed.columns:
            ...
            df_filtered_view = df_bets[df_bets['場次'].astype(str) == selected_view_match]
            if not df_filtered_view.empty and '人名' in df_filtered_view.columns:
    ...
    with tab4:
        ...
        if not merged.empty and '人名' in merged.columns:
            # 由於之前勝率邏輯較複雜，此處使用精簡安全版以防出錯
            player_stats = {player: {'win_full': 0, 'win_half': 0, 'total_valid': 0} for player in all_players}
            
            for index, row in merged.iterrows():
                player_name = str(row['人名']).strip()
            ...
            if stats_list:
                ...
                st.dataframe(df_stats[['勝率排名', '人名', '總有效投注', '勝出場數', '實際勝率']], use_container_width=True, hide_index=True)
            else:
                st.info("無有效結算數據。")
        else:
        ...
        if type(u) is str and u.strip() != "":
            ...
            if not merged.empty and '人名' in merged.columns:
                ...
                if player_history.empty:
                    ...
                    final_view['對賽場次'] = player_history['場次'].values if '場次' in player_history.columns else "未知"
                    ...
                    if target_res_col in player_history.columns:
                        final_view['賽果分類'] = player_history[target_res_col].fillna("未開賽/進行中").values
                    
                    if '得分' in player_history.columns:
...
# ==========================================
    ...
    st.error("🚨 系統發生錯誤！已經啟動安全攔截模式，網頁不會崩潰。")
    st.error(f"錯誤訊息: {str(e)}")
    st.warning("👇 請將下方紅色的 Error Code 截圖給我看，我馬上知道係邊度出錯。")
