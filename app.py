...
def load_sheet_data(worksheet_name):
    ...
    try:
        df = pd.read_csv(url)
        # Google Form 寫入 Bets 時通常第一欄係時間戳記，自動跳過
...
# ================= 1. 處理賽程與自動封盤狀態 =================
active_matches = []  # 未過期、可以買嘅場次
match_status_dict = {} # 紀錄每場波嘅狀態
    ...
    with st.sidebar.form(key="bet_form", clear_on_submit=True):
        ...
        if submit_bet:
            if bet_user == "選擇你的名字":
                ...
                if time_lock:
                    st.sidebar.error("❌ 呢場波已經開咗喇！唔準偷雞，落注失敗！")
                else:
                    # 🚀 背景自動射落後台 (利用 Google Form Web 協議)
                    ...
                    try:
                        ...
                        st.sidebar.success(f"🎉 落注成功！【{bet_user}】買咗【{bet_match} - {bet_side}】")
                        st.cache_data.clear()
                    except:
                        # 備用安全傳送門
                        ...
                        st.sidebar.warning("⚠️ 網絡稍慢，請點擊下方確認按鈕：")
...
with tab1:
    ...
    if not players_list:
        st.warning("⚠️ 目前 Players 分頁未有人名。")
    else:
        ...
        if not df_rank.empty:
            ...
            if lowest_score < 0: 
                st.error(f"🚨 目前由 **{lowest_player}** 以 {lowest_score} 分領先成為【終極大燈閪】！")
